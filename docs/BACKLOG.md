...
- ✅ **A1 · Merge `fix/tulong-mobile-structure` → `main`** — HECHO 2026-06-22. Mergeada y pusheada a `origin/main`. Bonus: el fix del fondo fluido de `a9eac82` (que solo estaba en CSS compilado) se hizo **durable en `scss/js-style/_fluid.scss`** para que el watcher no lo revierta. Luego `feat/cursor-arrow-gooey` también unificada en `main`.
